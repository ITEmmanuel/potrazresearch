#!/usr/bin/env python3
"""
Migration script to add processed_at column to Document table
"""
...
def add_processed_at_column():
    """Add processed_at column to Document table"""

    app = create_app()

    with app.app_context():
        print("Adding processed_at column to Document table...")
        ...
        try:
            ...
            with db.engine.connect() as conn:
                ...
                if 'mysql' in str(db.engine.url):
                    # MySQL syntax
                    conn.execute(db.text("ALTER TABLE document ADD COLUMN processed_at DATETIME NULL"))
                else:
                    # SQLite syntax
                    conn.execute(db.text("ALTER TABLE document ADD COLUMN processed_at DATETIME"))
                conn.commit()

            print("✅ Successfully added processed_at column to Document table")
